...
class RobotController:
    ...
    def _receive(self, answer_list, delay=0.00001):
    ...
    def exchange_msg(self, cmd, delay=0.00001, decode=True):
        ...
        if(not self.error):                                 #if there is no error
            ...
            #self.disconnect() I have commented the below code, Keerthi- looks stupid and is slowing it down
            #time.sleep(0.00001)
            #self.connect()
    ...
    def _decode_msg(self, response, response_key):
        ...
        if len(response) > 25:
            ...
            resp_cutoff = response[7:19]
            #print('beffore cutting off',resp_cutoff)
            if (resp_cutoff[-1] == ']'):
                resp_cutoff = response[7:18]
            else:
                resp_cutoff = response[7:20]

            #print('response_cut off is',resp_cutoff)
            
            #print('response_key is',response_key)
            code = None
            code = resp_cutoff
            response_key = int(response[1:4])
        else:
            code = None
        ...
        if(response_key == 2026 or response_key == 2027):   #if expected packet is from GetJoints (2026) or GetPose (2027), rest of packet is position data
            ...
            #self.mutex.acquire()
            #print('code_list is',code_list)
            code_list_int = tuple((int(x) for x in code_list))          #convert status data into integers - I changed to float - keerthi
            #self.mutex.release()
    ...
    def MoveJoints(self, theta_1, theta_2, theta_3, theta_4, theta_5, theta_6):
        ...
        #end = time.time()
        #print(end - start)
        return self.exchange_msg(cmd,0.00001)
    ...
    def MoveLinVelWRF(self, vx, vy, vz, wx, wy, wz):
        """Moves in velocity the Mecademic Robot world reference.
	...
	linear velocity mm/s angualr velocity deg/s
        ...
        """
        raw_cmd = 'MoveLinVelWRF'
        cmd = self._build_command(raw_cmd,[vx,vy,vz,wx,wy,wz])
        return self.exchange_msg(cmd)
    
    def MoveLinVelTRF(self, vx, vy, vz, wx, wy, wz):
        """Moves in velocity the Mecademic Robot world reference.
